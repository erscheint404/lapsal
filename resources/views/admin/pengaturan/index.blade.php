...
@section('content')
<div class="bg-white rounded-2xl border border-dark-100 shadow-sm overflow-hidden max-w-3xl">
    <div class="p-6 border-b border-dark-100">
        <h2 class="font-bold text-lg text-dark-900">Konfigurasi Umum</h2>
        <p class="text-sm text-dark-500 mt-1">Ubah nama, kontak, dan jam operasional aplikasi.</p>
    </div>

    <form action="{{ route('admin.pengaturan.update') }}" method="POST" class="p-6 space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="pengaturan[alamat]" rows="3" class="form-input" required>{{ \App\Models\Pengaturan::getValue('alamat', 'Jl. Futsal No. 1, Jakarta') }}</textarea>
            </div>
            
            <div>
                <label class="form-label">No. Telepon / WhatsApp</label>
                <input type="text" name="pengaturan[telepon]" value="{{ \App\Models\Pengaturan::getValue('telepon', '08123456789') }}" class="form-input" required>
            </div>
            ...
            <div class="md:col-span-2 border-t border-dark-100 pt-6 mt-2">
        ...
        <div class="pt-6 border-t border-dark-100 text-right">
            <button type="submit" class="btn-primary"><i class="fas fa-save mr-2"></i>Simpan Pengaturan</button>
        </div>
    </form>
</div>
@endsection
