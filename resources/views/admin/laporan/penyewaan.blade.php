...
<div class="card-premium overflow-hidden mb-6">
    <div class="p-7 border-b border-dark-100/80 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <form action="{{ route('admin.laporan.penyewaan') }}" method="GET" class="flex flex-wrap gap-4 w-full md:w-auto">
            <div class="flex items-center gap-2">
                ...
                <input type="date" name="start_date" value="{{ $startDate }}" class="form-input py-2.5">
            </div>
            <div class="flex items-center gap-2">
                <label class="text-sm font-bold text-dark-500">Sampai:</label>
                <input type="date" name="end_date" value="{{ $endDate }}" class="form-input py-2.5">
            </div>
            <button type="submit" class="btn-primary py-2.5 shadow-lg shadow-primary-500/25"><i class="fas fa-filter mr-2"></i>Filter Laporan</button>
        </form>
        <div class="flex gap-3">
            <a href="{{ route('admin.laporan.penyewaan.excel', ['start_date' => $startDate, 'end_date' => $endDate]) }}" class="btn-secondary py-2.5 print:hidden shadow-sm">
    ...
    <div class="p-7 bg-dark-50/50 border-b border-dark-100/80 print:bg-white print:p-0 print:border-none">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 print:grid-cols-4 print:gap-2">
            <div class="bg-white p-5 rounded-2xl border border-dark-100/80 shadow-sm">
        ...
        <table class="w-full text-left print:text-sm">
            ...
            <tbody class="divide-y divide-dark-100/80 print:divide-dark-200">
                ...
                <tr class="hover:bg-dark-50/50 transition-colors">
                    <td class="py-3 px-4 text-sm">{{ $booking->tanggal->format('d/m/Y') }}</td>
                    ...
                    <td class="py-3 px-4 text-sm">{{ substr($booking->jam_mulai, 0, 5) }} - {{ substr($booking->jam_selesai, 0, 5) }}</td>
                    <td class="py-3 px-4 text-sm">{{ $booking->durasi_jam }} Jam</td>
                    <td class="py-3 px-4"><span class="badge {{ $booking->status_color }} text-xs">{{ $booking->status_label }}</span></td>
                </tr>
                @empty
                <tr>
                    <td colspan="7" class="py-12 text-center text-dark-500">Tidak ada data penyewaan pada periode ini.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
