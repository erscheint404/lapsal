<header class="sticky top-0 z-30 transition-all duration-300"
    ...
    <div class="flex items-center justify-between px-6 lg:px-8 py-3.5">
        ...
        <div class="flex items-center gap-4">
            <button @click="sidebarOpen = true" class="w-10 h-10 flex items-center justify-center rounded-xl text-secondary-500 hover:text-secondary-900 transition-colors lg:hidden"
            ...
            <div class="hidden md:flex relative group">
                ...
                <input type="text" class="w-64 py-2.5 pl-11 pr-4 text-sm rounded-2xl transition-all"
                       ...
                       onfocus="this.style.background='white';this.style.borderColor='rgba(16,185,129,0.5)';this.style.boxShadow='0 0 0 4px rgba(16,185,129,0.1)'"
        ...
        <div class="flex items-center gap-3">
            <div class="relative" x-data="{ open: false }">
                <button @click="open = !open" @click.away="open = false" class="relative w-10 h-10 flex items-center justify-center rounded-xl text-dark-500 hover:text-dark-900 transition-colors"
                    ...
                    <span class="absolute top-2.5 right-2.5 w-2.5 h-2.5 rounded-full ring-2 ring-white" style="background: #34d399;"></span>
                </button>
            ...
            <div class="relative" x-data="{ open: false }">
                <button @click="open = !open" @click.away="open = false" class="flex items-center gap-3 focus:outline-none p-1.5 pr-3 rounded-2xl transition-all"
                    ...
                    <div class="text-right hidden sm:block">
                        <p class="text-sm font-bold text-secondary-900 leading-none">{{ auth()->user()->name }}</p>
                        <p class="text-xs mt-1 capitalize font-semibold" style="color: #059669;">{{ auth()->user()->role->nama }}</p>
                    </div>
                    <img src="{{ auth()->user()->avatar_url }}" class="w-10 h-10 rounded-xl object-cover shadow-sm shrink-0" style="border: 2px solid rgba(16,185,129,0.3);">
                ...
                <div x-show="open" x-transition.opacity.duration.200ms
                    ...
                    <div class="px-4 py-3" style="border-bottom: 1px solid rgba(0,0,0,0.06); background: rgba(0,0,0,0.02);">
                        <p class="font-bold text-secondary-900 text-sm">{{ auth()->user()->name }}</p>
                        <p class="text-xs capitalize font-semibold" style="color: #059669;">{{ auth()->user()->role->nama }}</p>
                    </div>
