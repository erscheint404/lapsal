...
<body class="font-sans text-secondary-700 bg-secondary-50 antialiased" x-data="{ sidebarOpen: false }">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="relative flex flex-col flex-1 overflow-y-auto overflow-x-hidden">
            ...
            <footer class="py-5 text-center text-sm text-secondary-400 mt-auto" style="border-top: 1px solid rgba(0,0,0,0.06); background: rgba(255,255,255,0.5); backdrop-filter: blur(10px);">
    ...
    <div x-show="sidebarOpen" x-transition.opacity style="display: none;"
         ...
         class="fixed inset-0 z-40 lg:hidden" style="background: rgba(15,23,42,0.6); backdrop-filter: blur(4px);">
    </div>

    @if(session('success'))
    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)"
         class="fixed bottom-6 right-6 px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 z-50 backdrop-blur-xl"
          style="background: rgba(255,255,255,0.92); border: 1px solid rgba(16,185,129,0.2);"
        ...
        <div class="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style="background: rgba(16,185,129,0.12);">
            <i class="fas fa-check-circle text-lg" style="color: #047857;"></i>
        </div>
        <p class="font-semibold text-sm text-secondary-900">{{ session('success') }}</p>
        <button @click="show = false" class="ml-2 p-1.5 rounded-lg hover:bg-secondary-100 transition-colors text-secondary-400">
    ...
    @if(session('error'))
    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)"
         class="fixed bottom-6 right-6 bg-white text-secondary-900 px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 z-50 border border-red-100/80 backdrop-blur-xl"
         x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="translate-y-8 opacity-0" x-transition:enter-end="translate-y-0 opacity-100"
         x-transition:leave="transition ease-in duration-200" x-transition:leave-start="translate-y-0 opacity-100" x-transition:leave-end="translate-y-8 opacity-0">
