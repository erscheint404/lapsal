...
<div class="bg-gradient-subtle py-12 min-h-[calc(100vh-80px)]">
    <div class="container-custom max-w-5xl">
        <div class="mb-10">
            <h1 class="text-3xl lg:text-4xl font-display font-black text-secondary-900 tracking-tight">Reschedule Jadwal</h1>
            <p class="text-secondary-500 mt-2 leading-relaxed">Ubah jadwal pesanan Anda. Anda hanya dapat memilih durasi yang sama ({{ $booking->durasi_jam }} jam).</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8" x-data="rescheduleWidget()">
            <div class="md:col-span-2 space-y-6">
                <div class="bg-white rounded-2xl border border-secondary-200/40 p-7 md:p-8">
                    ...
                    <div class="flex gap-5 items-start mb-6">
                        ...
                        <div>
                            <h4 class="text-xl font-bold text-secondary-900">{{ $lapangan->nama }}</h4>
                ...
                <div class="bg-white rounded-2xl border border-secondary-200/40 p-7 md:p-8">
                    <div class="flex justify-between items-end mb-5 pb-4 border-b border-secondary-100/80">
                        <h3 class="text-lg font-bold text-secondary-900">Pilih {{ $booking->durasi_jam }} Slot Jam</h3>
                        <span class="text-sm text-secondary-500 font-medium"><span x-text="selectedSlots.length"></span>/{{ $booking->durasi_jam }} dipilih</span>
                    </div>
                                ...
                                @if($slot->status === 'available' || ($slot->status === 'reserved' && $booking->tanggal->format('Y-m-d') == $tanggal && $slot->jam_mulai >= $booking->jam_mulai && $slot->jam_selesai <= $booking->jam_selesai))
                                    <label class="cursor-pointer block">
                                        ...
                                        <div class="px-3 py-3 text-center text-sm font-medium rounded-2xl border-2 transition-all duration-200
                                            ...
                                            bg-white border-secondary-200/80 text-secondary-600 hover:border-secondary-300">
                                            <span>{{ substr($slot->jam_mulai, 0, 5) }} - {{ substr($slot->jam_selesai, 0, 5) }}</span>
                                        </div>
                                    </label>
                                @else
                                    <div class="px-3 py-3 text-center text-sm font-medium rounded-2xl border-2 border-secondary-100/80 bg-secondary-50 text-secondary-300 cursor-not-allowed opacity-60">
                                        <span>{{ substr($slot->jam_mulai, 0, 5) }} - {{ substr($slot->jam_selesai, 0, 5) }}</span>
                                    </div>
                                @endif
                            @empty
            ...
            <div class="md:col-span-1">
                <div class="bg-white rounded-2xl border border-secondary-200/40 p-7 sticky top-24 shadow-xl">
                    ...
                    <div class="space-y-3 mb-6">
                        <div class="flex justify-between text-sm text-secondary-600">
                            ...
                            <span class="font-semibold text-secondary-900">Rp {{ number_format($booking->total_harga, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between text-sm text-secondary-600 font-bold text-amber-600">
                            <span>Biaya Reschedule (5%)</span>
                            <span>+ Rp {{ number_format($booking->total_harga * 0.05, 0, ',', '.') }}</span>
                        </div>
                        <p class="text-xs text-secondary-400 mt-3 italic leading-relaxed bg-amber-50/50 p-3 rounded-xl border border-amber-100/80">Biaya reschedule sebesar 5% dibayarkan langsung di kasir lapangan saat Anda datang bermain.</p>
                    </div>
